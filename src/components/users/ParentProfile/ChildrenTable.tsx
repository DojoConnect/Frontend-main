...
export default function ChildrenTable({ childrenData, onExport, currentPage = 1, rowsPerPage = 20, totalRows = 0, onPageChange, loading = false, parentId, onChildCreated }: ChildrenTableProps) {
 ...
 const handleInput = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
  ...
  return (
    <>
      <SearchActionBarCreateNew onExport={onExport} parentId={parentId} onCreated={onChildCreated} />
            ...
            {childrenData.map((child) => (
              <tr key={child.id} className="hover:bg-gray-50 cursor-pointer">
                ...
                <td className="flex items-center gap-2 px-4 py-3">
                  <Avatar src={child.avatar || null} alt={child.name} size={32} className="w-8 h-8 rounded-full" />
                  {child.name}
                </td>
                <td className="px-4 py-3">{child.email}</td>
                <td className="px-4 py-3">{child.enrolledDate ? formatDateCustom(child.enrolledDate) : ''}</td>
                <td className="px-4 py-3">{
                  (child.updatedAt || child.lastActivity || child.lastActivityDate)
                    ? formatDateCustom((child.updatedAt || child.lastActivity || child.lastActivityDate)!)
                    : ''
                }</td>
                <td className="px-4 py-3">{
                  child.status ? (
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${child.status === 'inactive' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                      {child.status}
                    </span>
                  ) : (
                    ''
      ...
      <div className="mt-4">
        <Pagination
          totalRows={totalRows || childrenData.length}
