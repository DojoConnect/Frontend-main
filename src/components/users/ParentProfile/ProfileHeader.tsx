...
// Map status values to their corresponding CSS classes
...
export default function ProfileHeader({ 
  ...
  return (
    <>
      ...
      <div className="flex md:items-center items-start gap-2 sm:gap-4 mb-4 sm:mb-6">
        <button
          onClick={onBack}
      ...
      <div className="flex items-center justify-between mb-6 sm:mb-8">
        <div className="flex items-center">
          <img
            src={profile.avatar || profile.avatarUrl || profile.classImg || undefined}
            alt={profile.name}
            className="w-12 h-12 sm:w-16 sm:h-16 rounded-full object-cover mr-3 sm:mr-6"
          />
          <div className="flex flex-col items-start">
            <div className="text-base sm:text-2xl font-bold">{profile.name}</div>
